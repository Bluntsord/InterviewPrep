...
class Solution:
    def minPathCost(self, grid: List[List[int]], moveCost: List[List[int]]) -> int:
        ...
        # init pq
        for starting_nodes in grid[0]:
            curr_tuple = (starting_nodes, 0, starting_nodes)
            self.visited[starting_nodes] = 0
            self.pq.put(curr_tuple)

        while not self.pq.empty():
            distance, level, curr = self.pq.get()
            if level == self.m - 1:
                return distance

            neighbours = grid[level + 1]

            for i in range(len(neighbours)):
                neighbour = neighbours[i]
                distance_to_neighbour = moveCost[curr][i] + distance + neighbour
                if neighbour not in self.visited or distance_to_neighbour < self.visited[neighbour]:
                    packaged_neighbour = (distance_to_neighbour, level + 1, neighbour)
                    self.pq.put(packaged_neighbour)
                    self.visited[neighbour] = distance_to_neighbour
...
grid3 = [[28,35,29,5,13,17,18,23,14],[30,15,12,27,2,26,25,19,7],[1,16,34,21,9,3,20,24,8],[4,33,22,11,31,0,6,10,32]]
...
solution = Solution()
print(solution.minPathCost(grid3, moveCost3 ))